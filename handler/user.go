...
func (h *UserHandler) ChangePasword(c echo.Context) error {
	u, code, errResponse := h.authenticateLogin(c)
	if u == nil {
		return c.JSON(code, errResponse)
	}

	hashPassword, err := u.HashPassword(c.QueryParam("password"))
	if err != nil {
		return c.JSON(http.StatusInternalServerError, utils.NewErrorResponse(http.StatusInternalServerError, utils.ErrorSeverity, "", err))
	}
