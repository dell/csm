...
Controller publish is taking too long to complete/ Health monitoring is causing the Unity array to panic by opening multiple sessions. As a workaround, Disable Volume health monitoring on the node and keep it only at the controller level. Refer [here](https://dell.github.io/csm-docs/docs/csidriver/features/unity/#volume-health-monitoring) for more information about enabling/disabling volume health monitoring.
